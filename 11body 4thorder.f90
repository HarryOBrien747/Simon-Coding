! 11body 4thorder.f90 — 11-body, 4th-order Hermite (jerk), plain-text output + LIVE PROGRESS BAR
program nbody_hermite_11
  ...
  real(dp), parameter :: dt  = 100.0_dp               ! 100s
  real(dp), parameter :: t_end = 600.0_dp*year         ! 600 years
  ...
  real(dp) :: vold(3,N)  
  real(dp) :: t, E0, Ecur, dErel, pct
  integer  :: step, nsteps, write_every, every_print
  ...
  ! Earth & Moon velocities set by init_earth_moon
  ...
  ! ---------- output (timestamped names) ----------
  call timestamp_tag(tag)
  state_name  = 'state_'  // trim(tag) // '.txt'
  ...
  t           = 0.0_dp
  nsteps      = int(t_end/dt)
  write_every = int(12.0_dp*3600.0_dp/dt)         ! every 12 hours to file
  every_print = max(1, int(0.01_dp*nsteps))       ! update progress ~100×

  ! announce + first write
  ...
  do step = 1, nsteps
     ...
     r    = r + 0.5_dp*(vold + v)*dt + (1.0_dp/12.0_dp)*(a - ap)*dt*dt
  ...
  call show_progress(nsteps, nsteps)  ! 100%
  write(*,*) ! newline after the bar
  close(ustate); close(uener)

contains
  ! ---- progress bar: prints one line and flushes immediately
  subroutine show_progress(i, total)
    ...
    write(output_unit,'(A)',advance='no') achar(13)// &
         '[', trim(bar), '] ', trim(adjustl(pct_str(100.0_dp*frac))), '%  (', &
         trim(adjustl(step_str(i))), '/', trim(adjustl(step_str(total))), ')'
  ...
  pure function step_str(k) result(s)
    integer, intent(in) :: k
    character(len=12)   :: s
    write(s,'(I0)') k
  end function step_str
  ...
  subroutine dump_state(u, N, t, r, v)
    ...
    write(u,'(ES25.16,*(1X,ES25.16))') line
