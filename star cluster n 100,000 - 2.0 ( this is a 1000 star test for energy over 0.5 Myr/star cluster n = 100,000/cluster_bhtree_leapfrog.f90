...
program cluster_bhtree_leapfrog
  ...
  real(dp), parameter :: theta = 0.2_dp            ! (unused here; kept for reference)

  ! Slightly tighter force criterion (helps true conservation a bit)
  real(dp), parameter :: theta_force  = 0.15_dp

  ! Much stricter energy diagnostic (cheap because energy is infrequent)
  real(dp), parameter :: theta_energy = 0.02_dp

  real(dp), parameter :: eps_m     = 1.0e15_dp     ! softening [m]; set 0 for none

  ! QUICK TEST SETTINGS (as requested):
  real(dp), parameter :: t_end_Myr = 0.5_dp        ! 0.5 Myr (half a million years)
  real(dp), parameter :: dt_Myr    = 0.0002_dp     ! timestep [Myr] (fixed)
  real(dp), parameter :: out_Myr   = 0.02_dp       ! snapshot cadence [Myr]

  integer, parameter :: seed0 = -12345             ! ran3 seed (negative = init)

  ! Pairwise energy verification (only for small N)
  ...
  if (N <= 0) N = 1000  ! default 1000 (as requested)
  ...
  call tree_accel_all(N, r, node_count, body, child, mass, com, half, eps2, theta_force, a)
  ...
  if (do_paircheck) then
    write(*,'(A)') 'Pairwise energy check: ON (small-N validation)'
  ...
  do step = 1, nsteps
     ...
     call tree_accel_all(N, r, node_count, body, child, mass, com, half, eps2, theta_force, anew)
...
contains
  ...
  subroutine write_mass_bins(tag, N, m_kg, Msun)
    ...
    integer, parameter :: nbin = nint((mmax - mmin)/dm)   ! 19 bins: [0.2,0.4) ... [3.8,4.0]
    integer :: counts(nbin)
    integer :: i, b
  ...
  subroutine tree_accel_all(N, r, node_count, body, child, mass, com, half, eps2, theta, a)
    ...
    !$omp parallel do default(none) shared(N,r,body,child,mass,com,half,eps2,theta2,a) &
    !$omp private(i,top,node,o,ch,b,stack,dx,r2,invr,invr3,s2) schedule(static)
    do i = 1, N
      a(:,i) = 0.0_dp
      top = 1
      stack(1) = 1
  ...
  subroutine tree_potential_all(N, r, node_count, body, child, mass, com, half, eps2, theta, phi)
    ...
    !$omp parallel do default(none) shared(N,r,body,child,mass,com,half,eps2,theta2,phi) &
    !$omp private(i,top,node,o,ch,b,stack,dx,r2,invr,s2) schedule(static)
    do i = 1, N
      phi(i) = 0.0_dp
      top = 1
      stack(1) = 1
